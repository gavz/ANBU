		...
		else if (syscall_number == SYS_NtResumeThread)
		{
			...
			for (it = process_data.begin(); it != process_data.end(); it++)
			{
				...
				for (size_t index_vector = 0; index_vector < it->second.size(); index_vector++)
				{
					...
					mem_dumper = new memory_dumper_t(it->second.at(index_vector).data);

					if (mem_dumper->parse_memory())
					{
				...
				if(!mem_dumper->dump_runpe_to_file(it->second, it->second.at(index_base_address).address))
				{
